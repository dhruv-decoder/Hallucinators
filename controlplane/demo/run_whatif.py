...
Runs the fixed support-bot workload under three risk appetites (strict / balanced / lenient)
and against an "oversight off" baseline, then prints how residual risk, escalation load, and net benefit
...
def policies() -> dict[str, PolicyProfile]:
    """Three risk appetites with a deterministic heuristics-only risk model.

    The risk model is held constant so only appetite varies: strict acts aggressively
    (low thresholds), lenient forwards more (high thresholds)."""
    return {
...
def main() -> None:
    simulator = WhatIfSimulator(
        synthetic_workload(),
        lambda policy: build_engine(policy, use_models=False),
    )
    ...
    print("Net benefit = cost saved - safety spend; positive is better.")
    ...
    print(
        ...
        f"at a net benefit of ${-balanced.net_usd:+.4f} "
        + ("(self-funding)" if balanced.net_usd < 0 else "(cost > savings)")
    )
