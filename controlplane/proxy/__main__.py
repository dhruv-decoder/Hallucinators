...
def main() -> None:
    ...
    # Durable by default: the server persists the hash-chained audit log to SQLite so the chain survives
    # restarts (an enterprise expectation). Override with CONTROLPLANE_RECORDER -- a .jsonl path for the
    # in-memory reference store, or a different .db path.
    recorder = os.environ.get("CONTROLPLANE_RECORDER", "controlplane.db")
